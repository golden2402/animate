...
async def get_all_watched_anime_by_user(user_id: str, response: Response):
    return await get_all_watched_anime_by_user(user_id=user_id)


@router.delete("/")
async def delete_watched_anime_for_user(watch: UpdateWatchedAnime, response: Response):
    await delete_watched_anime_relation(
        user_id=watch.user_id, anime_id=watch.episode_id
    )
    response.status = 200
    return response
...
@router.post("/")
async def watch_episode(watch: UpdateWatchedAnime, response: Response):
